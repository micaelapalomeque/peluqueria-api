...
function CuentaCorriente() {
  ...
  function enviarRecordatorio(deuda) {
    ...
    const numero  = celular.startsWith("54") ? celular : `54${celular}`
    const fecha   = new Date(deuda.fecha_turno).toLocaleDateString("es-AR", { weekday:"long", day:"numeric", month:"long" })
  ...
  const deudasFiltradas = deudas.filter(d =>
    (d.cliente_nombre || nombreCliente(d.cliente_id)).toLowerCase().includes(busquedaDeudas.toLowerCase()) ||
    String(d.turno_id).includes(busquedaDeudas)
  )
  ...
  const pagosFiltrados = pagos.filter(p =>
    nombreCliente(p.cliente_id).toLowerCase().includes(busquedaPagos.toLowerCase()) ||
    p.metodo_pago.toLowerCase().includes(busquedaPagos.toLowerCase()) ||
    p.tipo_pago.toLowerCase().includes(busquedaPagos.toLowerCase())
  )

  // ── Paginación ──
  const totalPaginasDeudas   = Math.ceil(deudasFiltradas.length / DEUDAS_POR_PAGINA)
  const deudasPagina         = deudasFiltradas.slice((paginaDeudas - 1) * DEUDAS_POR_PAGINA, paginaDeudas * DEUDAS_POR_PAGINA)
  ...
  const pagosPagina          = pagosFiltrados.slice((paginaPagos - 1) * PAGOS_POR_PAGINA, paginaPagos * PAGOS_POR_PAGINA)
      ...
      {cargando ? (
        ...
        <>
          ...
          {pestaña === "deudas" && (
            <>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"12px" }}>
                <p style={{ fontSize:"14px", fontWeight:500, color: TEMA.textoPrimario, margin:0 }}>Deudas pendientes</p>
                <BtnExportar
                  nombreArchivo="deudas_pendientes"
                  titulo="Deudas Pendientes"
                  columnas={["Cliente", "Turno", "Servicio", "Estado", "Monto"]}
                  filas={deudasFiltradas.map(d => [
              ...
              <div style={{ position:"relative", marginBottom:"12px" }}>
                ...
                <input
                  value={busquedaDeudas}
                  onChange={e => { setBusquedaDeudas(e.target.value); setPaginaDeudas(1) }}
                  placeholder="Buscá por nombre de cliente o número de turno..."
                ...
                {deudasPagina.length === 0 ? (
                  ...
                  >
                    ...
                    <div style={{ flex:2, fontSize:"12px", color: TEMA.textoSecundario }}>
                      ...
                      {deuda.fecha_turno && (
                        <p style={{ margin:0, fontSize:"11px", color: TEMA.textoTerciario }}>
                          {new Date(deuda.fecha_turno).toLocaleDateString("es-AR", { day:"numeric", month:"short", year:"numeric" })}
                        </p>
                      )}
                    </div>
              ...
              {totalPaginasDeudas > 1 && (
                <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:"12px", marginTop:"1rem" }}>
                  <button onClick={() => setPaginaDeudas(p => Math.max(1, p - 1))} disabled={paginaDeudas === 1}
                    style={{ padding:"6px 14px", borderRadius:"6px", border:`0.5px solid ${TEMA.borde}`, background: TEMA.superficie, color: paginaDeudas === 1 ? TEMA.textoDeshabilitado : TEMA.textoSecundario, cursor: paginaDeudas === 1 ? "not-allowed" : "pointer", fontSize:"13px" }}>←</button>
                  ...
                  <button onClick={() => setPaginaDeudas(p => Math.min(totalPaginasDeudas, p + 1))} disabled={paginaDeudas === totalPaginasDeudas}
                    style={{ padding:"6px 14px", borderRadius:"6px", border:`0.5px solid ${TEMA.borde}`, background: TEMA.superficie, color: paginaDeudas === totalPaginasDeudas ? TEMA.textoDeshabilitado : TEMA.textoSecundario, cursor: paginaDeudas === totalPaginasDeudas ? "not-allowed" : "pointer", fontSize:"13px" }}>→</button>
                </div>
              )}
              <p style={{ fontSize:"12px", color: TEMA.textoTerciario, textAlign:"center", marginTop:"8px" }}>
                {deudasFiltradas.length} deudas en total
              </p>
            </>
          )}
          ...
          {pestaña === "historial" && (
            <>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"12px" }}>
                <p style={{ fontSize:"14px", fontWeight:500, color: TEMA.textoPrimario, margin:0 }}>Historial de pagos</p>
                <BtnExportar
              ...
              <div style={{ position:"relative", marginBottom:"12px" }}>
                ...
                <input
                  value={busquedaPagos}
                  onChange={e => { setBusquedaPagos(e.target.value); setPaginaPagos(1) }}
                  placeholder="Buscá por nombre de cliente, método o tipo de pago..."
                  style={{ width:"100%", padding:"10px 12px 10px 36px", background: TEMA.superficie, border:`0.5px solid ${TEMA.borde}`, borderRadius:"6px", color: TEMA.textoPrimario, fontSize:"14px", boxSizing:"border-box" }}
                />
              </div>
              ...
              {totalPaginas > 1 && (
                <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:"12px", marginTop:"1rem" }}>
                  <button onClick={() => setPaginaPagos(p => Math.max(1, p - 1))} disabled={paginaPagos === 1}
                    style={{ padding:"6px 14px", borderRadius:"6px", border:`0.5px solid ${TEMA.borde}`, background: TEMA.superficie, color: paginaPagos === 1 ? TEMA.textoDeshabilitado : TEMA.textoSecundario, cursor: paginaPagos === 1 ? "not-allowed" : "pointer", fontSize:"13px" }}>←</button>
                  <span style={{ fontSize:"13px", color: TEMA.textoSecundario }}>
                  ...
                  <button onClick={() => setPaginaPagos(p => Math.min(totalPaginas, p + 1))} disabled={paginaPagos === totalPaginas}
                    style={{ padding:"6px 14px", borderRadius:"6px", border:`0.5px solid ${TEMA.borde}`, background: TEMA.superficie, color: paginaPagos === totalPaginas ? TEMA.textoDeshabilitado : TEMA.textoSecundario, cursor: paginaPagos === totalPaginas ? "not-allowed" : "pointer", fontSize:"13px" }}>→</button>
                </div>
              )}
              <p style={{ fontSize:"12px", color: TEMA.textoTerciario, textAlign:"center", marginTop:"8px" }}>
                {pagosFiltrados.length} pagos en total
              </p>
            </>
          )}
        </>
